  ...
  /*Active link*/
  const navLink = document.querySelectorAll('.contents-link a');   
...
//Graphql query and handlers and date handlers
  ...
  const populateTemplate = (repos) => {
    ...
    repositoryNode.innerHTML = `
      ...
      <h4><a href="${repos.url}">${repos.name ? repos.name : ""}</a></h4>
      ${repos.parent ? 
  ...
  <a href ="${repos.description ? repos.description : ""}">${repos.description ? repos.description : ""}</a>
</p>  <div class="repo-project">

  <p class="repo-project-language">
  ${repos.primaryLanguage ? 
    '<span class="language"><span class="language__circle" style="width:20px;color:' + repos.primaryLanguage.color + '; background-color:' 
...
<span> ${repos.stargazerCount ?
    '<span class="stars"><a href="https://github.com/achiever667' + repos.name + '/stargazers">' +
  ...
  ${repos.forkCount || (repos.parent ? repos.parent.forkCount: undefined) ?
    '<span class="pulls">' +
      ...
      '<svg aria-label="fork" viewBox="0 0 16 16" version="1.1" width="16" height="16" role="img"><path fill="#6a737d" fill-rule="evenodd" d="M5 3.25a.75.75 0 11-1.5 0 .75.75 0 011.5 0zm0 2.122a2.25 2.25 0 10-1.5 0v.878A2.25 2.25 0 005.75 8.5h1.5v2.128a2.251 2.251 0 101.5 0V8.5h1.5a2.25 2.25 0 002.25-2.25v-.878a2.25 2.25 0 10-1.5 0v.878a.75.75 0 01-.75.75h-4.5A.75.75 0 015 6.25v-.878zm3.75 7.378a.75.75 0 11-1.5 0 .75.75 0 011.5 0zm3-8.75a.75.75 0 100-1.5.75.75 0 000 1.5z"></path></svg>' +
        '&nbsp;'+ (repos.forkCount || repository.parent.forkCount) +
      '</a>' +
    '</span>':
    ""
  }
...
<div>
  ...
  const getUserData = (object) => {
    document.querySelector(".repo-total-count").textContent =object.repositories.edges.length
    document.querySelector(".name").textContent = object.name;
    document.querySelector(".description").textContent = object.bio;
    document.querySelectorAll(".profil-img").forEach( img => img.src = object.avatarUrl);
  ...
  fetch("https://api.github.com/graphql", {
    method: 'POST', 
    headers:{Authorization: "bearer  daa3011e52cd7b01fbc877a94fb286d337c53f5e"}, 
